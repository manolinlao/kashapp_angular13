...
export class LoadDenomsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.presentLoading(
      this.translateService.instant('loadDenoms.loadingBillsDenomination')
    );

    try {
      this.signalingSubscriptor = this.signalingService
        .getSubject()
        .subscribe((resp) => {
          ...
          if (
            objResp.type === 'new-client-atm' &&
            objResp.name === this.signalingService.connectedAtm
          ) {
          ...
          try {
            if (
              objResp.device === 'CASH_DISPENSER' &&
              objResp.type === 'AvailableBills'
            ) {
              this.dataAppService.setMaxBillsDispensables(
                objResp.maxBillsDispensables
              );
              const { bills } = objResp;
